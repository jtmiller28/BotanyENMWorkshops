---
title: "04 Accessible Area and Variable Selection"
...
#### Build an Accessible Area (AA), then crop climatic layers to it
...
To do this, we'll run a for-loop that processes each species AA using the package rangeBuilder's getDynamicAlphaHull(), crops + masks the climatic raster layers to these AA shapes, then deposits them in a directory called Cropped. 
```{r warning=FALSE}
...
for (species in unique(alldf$accepted_name)) {
  ...
  hull <- rangeBuilder::getDynamicAlphaHull(
    ...
    initialAlpha = 20, # initial alpha size, 20m, scales up from here. 
